 ...
 */
export async function recordRateLimit({ windowMs, keyPrefix = "rate_limit" }: Pick<RateLimitConfig, "windowMs" | "keyPrefix">): Promise<void> {
  const now = Date.now();
  const windowStart = Math.floor(now / windowMs) * windowMs;
  const key = `${keyPrefix}:${windowStart}`;

  try {
    // effectively open a transaction with Redis
...
export async function checkRateLimit({
  ...
  try {
    ...
    return {
      ...
      totalHits: 0,
      retry: async () => Promise.resolve(false),
    };
  }
}
