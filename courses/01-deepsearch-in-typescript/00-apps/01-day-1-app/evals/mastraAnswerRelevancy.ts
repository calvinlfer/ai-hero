...
// Scoring (post statement)
const ANSWER_RELEVANCY_AGENT_SYSTEM_PROMPT = `You are a balanced and nuanced answer relevancy evaluator. Your job is to determine if LLM outputs are relevant to the input, including handling partially relevant or uncertain cases.
...
function scoringPrompt({ input, statements }: {
  input: string;
  statements: string[];
}): string {
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object } = await generateObject({
    schema: z.object({
      statements: z.array(z.string().describe("A single statement"))
        .describe("A list of statements that make up the submission"),
    }),
    model: factualityModel,
    prompt: generateEvaluationStatementsPrompt({ output: opts.submission }),
  })
  const statements: string[] = object.statements;

  if (statements.length == 0) {
    return {
      score: 0,
      metadata: {
        reason: "No statements generated",
      },
    };
  } else {
    // assess relevancy of each statement
    const { object: verdictsObject } = await generateObject({
      system: ANSWER_RELEVANCY_AGENT_SYSTEM_PROMPT,
      schema: z.object({
        verdicts: z.array(
          z.object({
            verdict: z.enum(["yes", "no", "unsure"]).describe("Verdict"),
            reason: z.string().describe("Detailed reason for verdict"),
          }),
        ).describe("Verdicts for each statement"),
      }),
      model: factualityModel,
      prompt: scoringPrompt({ input: opts.question, statements }),
    })
    const scores = {
      yes: 1,
      no: 0,
    ...
    const totalScore = verdictsObject.verdicts.reduce((acc, next) => acc + scores[next.verdict], 0)
    const averageScore = totalScore / verdictsObject.verdicts.length

    return {
      score: averageScore,
      metadata: {
        statements,
        verdicts: verdictsObject.verdicts,
      },
    };
  }
}
...
export const MastraAnswerRelevancy = createScorer<string, string, string>({
